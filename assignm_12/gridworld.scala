...
object GridWorld {
  ...
  def generateGridWorld(xSize: Int, ySize: Int): Array[Array[Char]] = {
    var result = Array.fill[Char](xSize, ySize)('X')

    result(0)(0) = 'S'

    makePathInGrid(result, 0, 0)
    
    result(lastFreedPosition._1)(lastFreedPosition._2) = 'G'

    result
  }
  
  /*
   * This might need some simplification
   */
  def getPossibleDirections(newGrid: Array[Array[Char]], xPos: Int, yPos: Int): List[Int] = {
    var possibleDirections: List[Int] = Nil;
    if ((xPos - 1 >= 0 && newGrid(xPos - 1)(yPos) == 'X') &&
      (xPos - 2 < 0 || newGrid(xPos - 2)(yPos) == 'X') &&
      (yPos - 1 < 0 || newGrid(xPos - 1)(yPos - 1) == 'X') &&
      (yPos + 1 >= newGrid(0).length || newGrid(xPos - 1)(yPos + 1) == 'X')) {
      possibleDirections = 3 :: possibleDirections
    }
    if ((yPos - 1 >= 0 && newGrid(xPos)(yPos - 1) == 'X') &&
      (yPos - 2 < 0 || newGrid(xPos)(yPos - 2) == 'X') &&
      (xPos - 1 < 0 || newGrid(xPos - 1)(yPos - 1) == 'X') &&
      (xPos + 1 >= newGrid.length || newGrid(xPos + 1)(yPos - 1) == 'X')) {
      possibleDirections = 0 :: possibleDirections
    }
    if ((xPos + 1 < newGrid.length && newGrid(xPos + 1)(yPos) == 'X') &&
      (xPos + 2 >= newGrid.length || newGrid(xPos + 2)(yPos) == 'X') &&
      (yPos - 1 < 0 || newGrid(xPos + 1)(yPos - 1) == 'X') &&
      (yPos + 1 >= newGrid(0).length || newGrid(xPos + 1)(yPos + 1) == 'X')) {
      possibleDirections = 2 :: possibleDirections
    }
    if ((yPos + 1 < newGrid(0).length && newGrid(xPos)(yPos + 1) == 'X') &&
      (yPos + 2 >= newGrid(0).length || newGrid(xPos)(yPos + 2) == 'X') &&
      (xPos - 1 < 0 || newGrid(xPos - 1)(yPos + 1) == 'X') &&
      (xPos + 1 >= newGrid.length || newGrid(xPos + 1)(yPos + 1) == 'X')) {
      possibleDirections = 1 :: possibleDirections
    }
    possibleDirections
  }
  
  var lastFreedPosition = (0,0)

  def makePathInGrid(newGrid: Array[Array[Char]], xPos: Int, yPos: Int) {
    lastFreedPosition = (xPos, yPos)

    var possibleDirections = getPossibleDirections(newGrid, xPos, yPos)
    while (possibleDirections != Nil) {
      Random.shuffle(possibleDirections).head match {
        case 0 => { // nach links
          newGrid(xPos)(yPos - 1) = ' '
          makePathInGrid(newGrid, xPos, yPos - 1)
        }
        case 1 => { // nach rechts
          newGrid(xPos)(yPos + 1) = ' '
          makePathInGrid(newGrid, xPos, yPos + 1)
        ...
        case 2 => { // nach unten
          newGrid(xPos + 1)(yPos) = ' '
          makePathInGrid(newGrid, xPos + 1, yPos)
        }
        case 3 => { // nach oben
          newGrid(xPos - 1)(yPos) = ' '
          makePathInGrid(newGrid, xPos - 1, yPos)
      ...
      possibleDirections = getPossibleDirections(newGrid, xPos, yPos)
    }
  }

  def generateGridWorldOld(xSize: Int, ySize: Int): Array[Array[Char]] = {
  ...
  private def printSituation(positionAgent: (Int, Int)) {
    printSituation(grid, positionAgent)
  }

  private def printSituation(thisGrid: Array[Array[Char]], positionAgent: (Int, Int)) {
    val before = thisGrid(positionAgent._1)(positionAgent._2)
    thisGrid(positionAgent._1)(positionAgent._2) = 'o'
    println
    thisGrid.foreach(row => { row.foreach(print); println; })
    thisGrid(positionAgent._1)(positionAgent._2) = before
    readLine
  }
