...
closest_test_labels = knnClassify(train_data, train_label, test_data);
closest_train_labels = knnClassify(train_data, train_label, train_data);
...
for it = 1:length(k_values)
    ...
    tic
    test_pred_label = mode(closest_test_labels(:,1:k_values(it)),2);
    error_test(it) = loss01(test_pred_label, test_label);
    toc
    
     disp(['Working on training data for k=' int2str(k_values(it))]);
     tic
     train_pred_label = mode(closest_train_labels(:,1:k_values(it)),2);
